...
- 캐시
  ...
  - 동일한 요청에 대해 캐시된 데이터 활용 가능
  - 불필요한 대역폭 낭비, 응답 지연 방지
  - 빠른 자원 접근 가능
  - 개인 전용 캐시: 웹 브라우저에 저장
  - 공용 캐시: 클라이언트와 서버 사이의 중간 서버에 저장
  - 캐시 신선도(cache freshness)
    - 캐시된 데이터가 얼마나 최신 데이터와 유사한지
    - 유효 기간 설정
      - 기간이 만료되면 원본 데이터 다시 요청
    - 재검사
      - If-Modified-Since 헤더
        - 날짜를 기반으로 서버에게 문의
        - 헤더에 명시된 시점 이후에 원본에 변경이 있으면 새 자원으로 응답하도록 요청
      - If-None-Match 헤더
        - 엔티티 태그(Etag)를 기반으로 서버에게 문의
        - Etag: 자원의 버전을 식별하기 위한 정보
        - 자원이 변경될 때마다 자원의 버전을 식별하는 Etag 값이 변경
        - 만약 Etag 값이 변경되었을 경우 새 자원으로 응답하도록 요청
      - Last-Modified 헤더
        - 상태 코드 304를 통한 자원의 변경 여부뿐 아니라 마지막으로 수정된 시점도 알려 줄 수 있음
- 쿠키
  - 서버에서 생성되어 클라이언트 측에 저장되는 데아터
  - 상태를 유지하지 않는 HTTP의 특성을 보완하기 위한 수단
  - 기본적으로 <이름, 값> 쌍의 형태이며 추가로 다양한 속성을 가질 수 있음
  - 응답 메시지의 Set-Cookie 헤더
    - 쿠키의 이름, 값과 더불어 ';'으로구분되는 속성들 전달
    - 한 응답 메시지에 전달할 쿠기가 여러 개라면 여러 개의 Set-Cookie 전달
  - 요청 메시지의 Cookie 헤더
    - 서버에 전달할 쿠키의 이름과 값을 나타내는 헤더
    - 여러 개의 쿠키 값을 서버에 전달핡할 경우 ';'을 사용해 여러 개의 Cookie 전달
  - 쿠키의 속성
    - 도메인
      - 쿠키는 사용 가능한 도메인이 정해져 있음
      - 응답 메시지 속 Set-Cookie 헤더의 'domain' 속성
      - 같은 도메인에 경로별로 쿠키를 구분하여 사용하고 싶을 경우 "path" 속성으로쿠키가 적용될 경로 명시
    - 유효 기간
      - Expires 속성을 통해 쿠키 만료 시점 지정
      - Max-Age 속성을 통해 초 단위 유효 기간 지정
  - 쿠키의 한계
    - 쿠키 정보는 쉽게 노출되거나 조작될 수 있음
    - Secure: HTTPS 프로토콜이 사용될 때만 쿠키 전송
    - HttpOnly: HTTP 송수신을 통해서만 쿠키를 이용하도록 제한
- 콘텐츠 협상
  - 같은 URI에 대해 가장 적합한 자원의 형태를 제공하는 기술
  - ex. 한국어로 요청하면 한국어 페이지, 영어로 요청하면 영어 페이지 제공
  - 콘텐츠 협상 관련 HTTP 헤더
    - Accept 헤더
      - 선호하는 미디어 타입
    - Accept-Language
...
### SSL과 TLS
- 인증과 암호화를 수행하는 프로토콜
- TLS는 SSL을 계승한 프로토콜로 작동 과정은 큰 틀에서 보면 유사함
- HTTPS
